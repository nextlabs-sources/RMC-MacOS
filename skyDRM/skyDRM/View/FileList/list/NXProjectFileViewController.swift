//
//  NXProjectFileViewController.swift
//  skyDRM
//
//  Created by William (Weiyan) Zhao on 2019/3/19.
//  Copyright © 2019 nextlabs. All rights reserved.
//

import Cocoa
import SDML
protocol NXProjectFileViewControllerDelegate: NSObjectProtocol {
    func openFolder(item:Any)
    func addFileToProject(nxlFile: NXProjectFileModel)
}
@objcMembers class NXProjectFileViewController:NSViewController{
    ...
    weak var delegate:NXProjectFileViewControllerDelegate?
    ...
    fileprivate func locationed() {
        emptyFolderLab.stringValue = "FILE_INFO_EMPTY_PROJECT".localized
    ...
    private func reloadData(isRefresh: Bool) {
        ...
        file.removeAll()
        folder.removeAll()
        for projectFile in data.rootFiles {
            if let newFolder = projectFile.file as? NXProjectFileModel {
        ...
        displayEmptyView(isShow: data.rootFiles.isEmpty)
        fileTableView.reloadData()
    }
}
// MARK: Sort.

extension NXProjectFileViewController {
    fileprivate func filterCompletion() {
            displayData = data.rootFiles
    }
    fileprivate func filterWithSearchString() {
        if searchString.isEmpty {
            displayData = data.rootFiles
        } else {
            let filterData = data.rootFiles.filter() {$0.file.name.localizedCaseInsensitiveContains(searchString)}
    ...
    fileprivate func sortContentOrderBy(_ sortOrder: NXListOrder, ascending: Bool) {
        self.displayData = getOrderFiles(files: self.folder, sortOrder: sortOrder, ascending: ascending) + getOrderFiles(files: self.file, sortOrder: sortOrder, ascending: ascending)
    }
...
extension NXProjectFileViewController {
    ...
    @objc func observeNotification(_ notification: Notification) {
        if notification.nxNotification == .projectProtected {
            if let projectFile = notification.object as? NXLocalProjectFileModel  {
                if projectFile.projectId != self.data.id {
                    return
                }
                
                if projectFile.parentFileId != nil && projectFile.parentFileId != "" {
                    return
                }
            ...
            if let syncFile = notification.object as? NXSyncFile,
                ...
                if projectFile.project?.id != self.data.id {
                    return
                }
                
                if projectFile.parentFileID != nil {
                    return
                }
    ...
    func getCurrentFile(file: NXSyncFile) -> NXSyncFile? {
        for (index, item) in self.data.rootFiles.enumerated() {
            if let nxFile = item.file.sdmlBaseFile as? SDMLNXLFile, let removeFile = file.file.sdmlBaseFile as? SDMLNXLFile {
                if nxFile.getDUID() == removeFile.getDUID() {
                    return self.data.rootFiles[index]
                }
            }
        }
        return nil
...
extension NXProjectFileViewController {
    ...
    func removeFileWithFile(file: NXSyncFile) {
        
        for (index, item) in self.data.rootFiles.enumerated() {
            if item == file {
                self.data.rootFiles.remove(at: index)
                self.displayEmptyView(isShow: self.data.rootFiles.isEmpty)
                break
            }
        }
        ...
        _ = NXClient.getCurrentClient().deleteFile(file: file.file, isUpload: false)
    }
    ...
    func updateProjectFile(file: NXSyncFile, newFile: NXSyncFile) {
        ...
        for (index, item) in self.data.rootFiles.enumerated() {
            if item == file {
                self.data.rootFiles[index] = newFile
                currentIndex = index
                break
            }
        }
...
extension NXProjectFileViewController: NSTableViewDataSource {
...
extension NXProjectFileViewController: NSTableViewDelegate {
...
extension NXProjectFileViewController: NXLocalListItemContextMenuOperationDelegate {
