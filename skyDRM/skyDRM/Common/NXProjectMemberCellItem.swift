...
class NXProjectMemberCellItem: NSCollectionViewItem {
    ...
    var memberInfo: NXProjectMember?{
        didSet{
            ...
            attributeStr.addAttribute(NSForegroundColorAttributeName, value:NSColor.lightGray, range: range)
            
            let pstyle = NSMutableParagraphStyle()
            pstyle.alignment = .center
            attributeStr.addAttributes([NSParagraphStyleAttributeName: pstyle], range: NSMakeRange(0, attributeStr.length))
    ...
    func setDefaultImage(member:NXProjectMember)
    {
        ...
        if let colorHexValue = NXCommonUtils.circleViewBKColor[(NXCommonUtils.abbreviation(forUserName: (displayName)).substring(to: index!).lowercased())]{
