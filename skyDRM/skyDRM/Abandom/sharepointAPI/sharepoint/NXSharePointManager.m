...
@property(nonatomic) BOOL formDigestToDelete;
@property(nonatomic) BOOL formDigestToCreate;
...
-(void) initRemoteQueryResponseHandler
{
    ...
    RemoteQueryResponseHandler getContextInfo = ^(NXSharePointRemoteQueryBase* query, NSData* data)
    {
        ...
        switch (rqType) {
            ...
            break;
    case kSPQueryDeleteItem:
        {
            NXFileBase *spFile=appendDic[SP_DICTION_TAG_BASEFILE];
            __strong NXSharePointManager *strongself =weakself;
            [strongself deleteFileOrFolder:spFile];
        }
        break;
    case kSPQueryCreateFolder:
            {
                ...
                NXFileBase *spFile=appendDic[SP_DICTION_TAG_BASEFILE];
                __strong NXSharePointManager *strongself =weakself;
                [strongself createFolderName:fileName withParent:spFile];
...
-(void) createFolderName:(NSString*)fileName withParent:(NXFileBase*)parentFolder {
       if (self.formDigestToCreate) {
        self.formDigestToCreate=NO;
        NSString *createFolder=nil;
        NSDictionary *bodyDic=[NSDictionary dictionary];
           if ([parentFolder.fullPath isEqualToString:@"/"]){
            createFolder=[NSString stringWithFormat:[NXSharePointRestTemp SPCreateListFolderTemp],self.siteURL];
            NSDictionary *bobyDic=@{@"__metadata":@{@"type": @"SP.List"},@"AllowContentTypes":@"true",@"BaseTemplate":@"101",@"ContentTypesEnabled":@"true",@"Title":fileName};
            bodyDic=bobyDic;
           }else  {
               NSString *urlStr =[self bringUpUrlStrForCreateFolderFromString:parentFolder.SPSiteId];
               if ([parentFolder isKindOfClass:[NXSharePointFolder class]]) {
                   NXSharePointFolder *folder=(NXSharePointFolder*)parentFolder;
                   if (folder.folderType== kSPSite) {
                       createFolder=[urlStr stringByAppendingPathComponent:@"/web/lists"];
                       NSDictionary *bobyDic=@{@"__metadata":@{@"type": @"SP.List"},@"AllowContentTypes":@"true",@"BaseTemplate":@"101",@"ContentTypesEnabled":@"true",@"Title":fileName};
                       bodyDic=bobyDic;
                   }else{
                        createFolder=[urlStr stringByAppendingPathComponent:@"/web/folders"];
                       NSString *decPath=[parentFolder.fullPath stringByAppendingPathComponent:fileName];
                       NSDictionary *bobyDic=@{@"__metadata":@{@"type":@"SP.Folder"},@"ServerRelativeUrl":decPath};
                       bodyDic=bobyDic;
                   }
             }
           }
        NXSharePointRemoteQueryBase *createBase =[self initializeQuery:createFolder];
        NSDictionary *headers =@{@"X-RequestDigest":_formDigestValue,@"accept":@"application/json;odata=verbose",@"content-type":@"application/json;odata=verbose"};
        NSError *error;
        NSData *bodyData = [NSJSONSerialization dataWithJSONObject:bodyDic options:NSJSONWritingPrettyPrinted error:&error];
         NSMutableDictionary* appendData = [[NSMutableDictionary alloc] initWithObjects:@[fileName, parentFolder.fullPath,parentFolder] forKeys:@[SP_DICTION_TAG_FILE_NAME,SP_DICTION_TAG_FOLDER_REV_URL,SP_DICTION_TAG_BASEFILE]];
        ...
        self.formDigestToCreate = YES;
        
        NSMutableDictionary* appendData = [[NSMutableDictionary alloc] initWithObjects:@[[[NSNumber alloc] initWithInteger:kSPQueryCreateFolder],fileName, parentFolder.fullPath,parentFolder] forKeys:@[SP_DICTION_TAG_REQ_TYPE, SP_DICTION_TAG_FILE_NAME, SP_DICTION_TAG_FOLDER_REV_URL,SP_DICTION_TAG_BASEFILE]];
        
        [self getContextInfo:appendData];
    }
    
}
-(void) deleteFileOrFolder:(NXFileBase*)spFile {
    if (self.formDigestToDelete) {
        self.formDigestToDelete=NO;
        NSString *deleteItem=nil;
        deleteItem=spFile.SPSiteId;
         NSDictionary *headers = @{@"X-RequestDigest":_formDigestValue,@"X-HTTP-Method":@"DELETE", @"IF-MATCH":@"*"};
//        if ([spFile isKindOfClass:[NXSharePointFile class]]) {
////             deleteItem =[NSString stringWithFormat:[NXSharePointRestTemp SPDeleteFileTemp],self.siteURL,spFile.SPSiteId];
//            
//        } else if([spFile isKindOfClass:[NXSharePointFolder class]]) {
////            if ([spFile.fullServicePath containsString:@"-"]) {
////                 deleteItem =[NSString stringWithFormat:[NXSharePointRestTemp SPDeleteListTemp],self.siteURL,spFile.fullPath];
////            }else
////            {
////                deleteItem =[NSString stringWithFormat:[NXSharePointRestTemp SPDeleteFolderTemp],self.siteURL,spFile.SPSiteId];
//            deleteItem=spFile.SPSiteId;
////        }
//        }
        NXSharePointRemoteQueryBase *deleteQuery = [self initializeQuery:deleteItem];
        NSMutableDictionary* appendData = [[NSMutableDictionary alloc] initWithObjects:@[spFile.name, spFile.fullServicePath,spFile] forKeys:@[SP_DICTION_TAG_FILE_NAME,SP_DICTION_TAG_FOLDER_REV_URL, SP_DICTION_TAG_BASEFILE]];
        
        [deleteQuery executeQueryWithRequestId:kSPQueryDeleteItem Headers:headers RequestMethod:@"POST" BodyData:nil withAdditionData:appendData];
    }else {
        self.formDigestToDelete = YES;
            
        NSMutableDictionary* appendData = [[NSMutableDictionary alloc] initWithObjects:@[[[NSNumber alloc] initWithInteger:kSPQueryDeleteItem],spFile.name, spFile.fullServicePath,spFile] forKeys:@[SP_DICTION_TAG_REQ_TYPE, SP_DICTION_TAG_FILE_NAME, SP_DICTION_TAG_FOLDER_REV_URL,SP_DICTION_TAG_BASEFILE]];
...
-(void) remoteQuery:(NXSharePointRemoteQueryBase*) spQuery didCompleteQuery:(NSData*) data
{
    ...
    if (spQuery.queryID==kSPQueryDeleteItem) {
        if ([self.delegate respondsToSelector:@selector(didDeleteItemFinishedFormPath:)]) {
            [self.delegate didDeleteItemFinishedFormPath:nil];
        }
        return;
    }else if (spQuery.queryID==kSPQueryCreateFolder) {
        NSString *newSPSiteId=[[NSString alloc]initWithData:data encoding:NSUTF8StringEncoding];
               if ([self.delegate respondsToSelector:@selector(didCreateFolderFinishedTopath:)]) {
            [self.delegate didCreateFolderFinishedTopath:newSPSiteId];
        }
        return;
    }
...
-(void) remoteQuery:(NXSharePointRemoteQueryBase*) spQuery didFailedWithError:(NSError*) error
{
    ...
    if (spQuery.queryID==kSPQueryDeleteItem) {
        DLog(@"删除失败sharepoint delete item fail error is %@",error);
        if ([self.delegate respondsToSelector:@selector(didDeleteItemFailWithError:)]) {
        [self.delegate didDeleteItemFailWithError:error];
        }
        return;
    }else if (spQuery.queryID==kSPQueryCreateFolder) {
        NSLog(@"创建文件夹失败");
        if ([self.delegate respondsToSelector:@selector(didCreateFolderFailwithError:)]) {
            [self.delegate didCreateFolderFailwithError:error];
        }
    }
...
-(NSString*)bringUpUrlStrForCreateFolderFromString:(NSString*)string{
    NSMutableString *muStr =[NSMutableString stringWithString:string];
    NSRange range = [muStr rangeOfString:@"/Web"];
    NSString *urlStr =[muStr substringToIndex:range.location];
       return urlStr;
...
-(void) resetQueryFlag
{
    ...
    _formDigestToDelete = NO;
    _formDigestToCreate = NO;
}
@end
