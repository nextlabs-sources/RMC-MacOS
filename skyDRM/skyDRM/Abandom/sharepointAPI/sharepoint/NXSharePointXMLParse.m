...
+(NSArray*) parseGetDocLibLists:(NSData*) data
{
    ...
    for (NXXMLElement* entryNode in entryArray) {
        NXXMLElement* idNode = [entryNode childNamed:SP_id_TAG];
            ...
            if (propertiesNode) {
                ...
                if (![hiddenNode.value isEqualToString:@"true"]) {
                    if (titleNode.value && idNode.value && createTimeNode.value && parentWebURL.value) {
                        ...
                        NSDictionary* dic = [NSDictionary dictionaryWithObjects:@[titleNode.value, SP_NODE_DOC_LIST, IdNode.value, createTimeNode.value, localString, idNode.value] forKeys:@[SP_TITLE_TAG, SP_NODE_TYPE, SP_ID_TAG, SP_CREATED_TAG, SP_PARENT_WEB_URL, SP_id_TAG]];
        ...
        if (contentNode) {
            ...
            if (propertiesNode) {
                ...
                if (nameNode.value && serverRelativeUrlNode.value) {
                    NSDictionary* dic = [NSDictionary dictionaryWithObjects:@[nameNode.value, serverRelativeUrlNode.value, SP_NODE_FOLDER, idNode.value] forKeys:@[SP_NAME_TAG, SP_SERV_RELT_URL_TAG, SP_NODE_TYPE, SP_id_TAG]];
                    [retArray addObject:dic];
        ...
        if (contentNode) {
            ...
            if (propertiesNode) {
                ...
                if (nameNode.value && serverRelativeUrlNode.value && contentVersionNode.value && fileLength.value && timeLastModified.value) {
                    NSDictionary* dic = [NSDictionary dictionaryWithObjects:@[nameNode.value, serverRelativeUrlNode.value, contentVersionNode.value, fileLength.value, SP_NODE_FILE, timeLastModified.value, idNode.value] forKeys:@[SP_NAME_TAG, SP_SERV_RELT_URL_TAG, SP_CONTENT_VERSION_TAG, SP_FILE_SIZE_TAG, SP_NODE_TYPE, SP_TIME_LAST_MODIFY, SP_id_TAG]];
                    [retArray addObject:dic];
                }
            }
        ...
        if (contentNode) {
            ...
            if (propertiesNode) {
                ...
                if (titleNode.value && siteURLNode.value && createdTimeNode.value && siteRelativeURLNode.value) {
                    NSDictionary* dic = [NSDictionary dictionaryWithObjects:@[titleNode.value, siteURLNode.value, SP_NODE_SITE, createdTimeNode.value, siteRelativeURLNode.value, idNode.value] forKeys:@[SP_TITLE_TAG, SP_URL_TAG, SP_NODE_TYPE, SP_CREATED_TAG, SP_SERV_RELT_URL_TAG, SP_id_TAG]];
                    [retArray addObject:dic];
