//
//  NXSpecificProjectHomePeopleCircleText.swift
//  skyDRM
//
...
class NXSpecificProjectHomePeopleCircleText: NSView {

    var circleText:NXCircleText?
    let itemSize:NSSize = NSSize(width: 56, height: 56)
    
    override init(frame frameRect: NSRect){
        super.init(frame: NSRect(origin: frameRect.origin, size: itemSize))
        // Do view setup here.
        circleText = NXCircleText()
        circleText?.frame = CGRect(x: 0, y: 0, width: 56, height: 56)
        ...
        let conWidth = circleText?.widthAnchor.constraint(equalToConstant: 56)
        let conHeight = circleText?.heightAnchor.constraint(equalToConstant: 56)
...
extension NXSpecificProjectHomePeopleCircleText:MouseEventDelegate{
