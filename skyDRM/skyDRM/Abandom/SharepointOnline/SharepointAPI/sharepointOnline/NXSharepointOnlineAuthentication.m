...
#define kSPUserUsername @"SPUserCodingUsername"
#define kSPUserPassword @"SPUserCodingPassword"
#define kSPUserSite @"SPUserCodingSite"
#define kSPUserFedauthInfo @"SPUserCodingFedauthInfo"
#define kSPUserRtfaInfo @"SPUserCodingRtfaInfo"
...
- (instancetype) initWithCoder:(NSCoder *)aDecoder{
    if (self = [super init]) {
        self.username = [aDecoder decodeObjectForKey:kSPUserUsername];
        self.password = [aDecoder decodeObjectForKey:kSPUserPassword];
        self.siteurl = [aDecoder decodeObjectForKey:kSPUserSite];
        self.fedauthInfo = [aDecoder decodeObjectForKey:kSPUserFedauthInfo];
        self.rtfaInfo = [aDecoder decodeObjectForKey:kSPUserRtfaInfo];
    }
    return self;
}

- (void)encodeWithCoder:(NSCoder *)aCoder{
    [aCoder encodeObject:_username forKey:kSPUserUsername];
    [aCoder encodeObject:_password forKey:kSPUserPassword];
    [aCoder encodeObject:_siteurl forKey:kSPUserSite];
    [aCoder encodeObject:_fedauthInfo forKey:kSPUserFedauthInfo];
    [aCoder encodeObject:_rtfaInfo forKey:kSPUserRtfaInfo];
}
@end
 ...
 completionHandler:(void (^)(NSURLRequest * __nullable))completionHandler
{
    ...
    if([responsestr rangeOfString:SP_FAULT_TAG].location != NSNotFound)
    {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([self->_delegate respondsToSelector:@selector(Authentication:didAuthenticateFailWithError:)]) {
                [self->_delegate Authentication:self didAuthenticateFailWithError:strerror];
            }
        });
    }
...
didCompleteWithError:(NSError *)error
{
    if(error)
    {
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([self->_delegate respondsToSelector:@selector(Authentication:didAuthenticateFailWithError:)]) {
                [self->_delegate Authentication:self didAuthenticateFailWithError:error.description];
            }
        });
    }
    else
    {
        if (_user.rtfaInfo && _user.fedauthInfo) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if([self->_delegate respondsToSelector:@selector(Authentication:didAuthenticateSuccess:)]){
                    [self->_delegate Authentication:self didAuthenticateSuccess:self->_user];
                }
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([self->_delegate respondsToSelector:@selector(Authentication:didAuthenticateFailWithError:)]) {
                [self->_delegate Authentication:self didAuthenticateFailWithError:error];
            }
        });
    }
}
